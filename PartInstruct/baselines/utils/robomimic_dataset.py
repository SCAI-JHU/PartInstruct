...
class RobomimicReplayImageDataset(BaseImageDataset):
    def __init__(self,
            ...
            with FileLock(cache_lock_path):
                ...
                if not os.path.exists(cache_zarr_path):
                    ...
                    try:
                        ...
                        replay_buffer = _convert_robomimic_to_replay(self,
        ...
        if n_obs_steps is not None:
            ...
            for key in rgb_keys + mask_keys + lowdim_keys:
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        data = {}
        for key in self.lowdim_keys+self.text_keys+['action']:
...
def _convert_robomimic_to_replay(self, store, shape_meta, dataset_path, abs_action, rotation_transformer, 
    ...
    with h5py.File(dataset_path, 'r+') as file:
        ...
        def img_copy(zarr_arr, zarr_idx, hdf5_arr, hdf5_idx):
            try:
                if zarr_arr[zarr_idx].shape != hdf5_arr[hdf5_idx].shape:
                    # deep copy 
                    data = hdf5_arr[hdf5_idx].copy()
                    zarr_arr[zarr_idx] = data.reshape(zarr_arr[zarr_idx].shape)
                else:
        ...
        with tqdm(total=n_steps*len(mask_keys), desc="Loading mask data", mininterval=1.0) as pbar:
            # one chunk per thread, therefore no synchronization needed
            with concurrent.futures.ThreadPoolExecutor(max_workers=n_workers) as executor:
                futures = set()
        ...
        with tqdm(total=n_steps*len(pcd_keys), desc="Loading pcd data", mininterval=1.0) as pbar:
            # one chunk per thread, therefore no synchronization needed
            with concurrent.futures.ThreadPoolExecutor(max_workers=n_workers) as executor:
                futures = set()
                for key in pcd_keys:
                    ...
                    for episode_idx in range(len(demos)):
                        ...
                        for hdf5_idx in range(hdf5_arr.shape[0] - 1):
                            if len(futures) >= max_inflight_tasks:
                                ...
                                for f in completed:
                                    if not f.result():
                                        raise RuntimeError('Failed to encode pcd')
                                pbar.update(len(completed))
