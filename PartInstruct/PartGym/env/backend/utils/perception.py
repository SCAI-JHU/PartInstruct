...
from PartInstruct.PartGym.env.backend.utils.transform import Transform
...
class TSDFVolume(object):
    ...
    def __init__(self, size, resolution, origin=np.array([0,0,0])):
        self.size = size
...
def randomly_sample_point(point_cloud, z_min=0.0):
    """
    Randomly sample a point from a point cloud with a constraint on the z-coordinate.

    Parameters:
        point_cloud (o3d.geometry.PointCloud): The point cloud to sample from.
        z_min (float): The minimum allowed z-coordinate for the sampled point.

    Returns:
        np.ndarray: The randomly selected point as a numpy array that meets the z-coordinate constraint,
                    or None if no points meet the criteria.
    """
    # Convert Open3D point cloud to numpy array
    points = np.asarray(point_cloud.points)
    
    # Filter points based on z-coordinate
    valid_points = points[points[:, 2] > z_min]
    
    if len(valid_points) == 0:
        print("No points found above the specified z-coordinate.")
        return None
    
    # Randomly select a point from the filtered list
    random_index = np.random.randint(len(valid_points))
    random_point = valid_points[random_index]
