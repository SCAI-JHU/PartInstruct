...
from PartInstruct.PartGym.env.backend.utils.grasp import *
from PartInstruct.PartGym.env.backend.utils.transform import Transform, Rotation
from .vgn_networks import load_network
...
class VGN(object):
    ...
    def __call__(self, state):
        ...
        grasps, scores = select(qual_vol.copy(), rot_vol, width_vol, threshold=0.75)
        toc = time.time() - tic
...
def select(qual_vol, rot_vol, width_vol, threshold=0.90):
    # threshold on grasp quality
